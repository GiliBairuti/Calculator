...
def test_complicated_1():
    assert 28.049999999999997 == Calculation.calculation_answer("~-6+7&4-1^43#+28.55-(12@7)")


def test_complicated_2():
    assert 253 == Calculation.calculation_answer("12/3!+23  4#$10  *5^2-23+~-4!")


def test_complicated_3():
    assert 11 == Calculation.calculation_answer("(5+7-12@8*5+6!)%(~4*13+100)+100#")


def test_complicated_4():
    assert 176 == Calculation.calculation_answer("-4+5%2@12+65^0--78+10&34-(42#$11)+100      -3")


def test_complicated_5():
    assert 1000033390 == Calculation.calculation_answer("1000034332+100-(3^ 10)&60000%675  -(6)!+2")


def test_complicated_6():
    assert 25 == Calculation.calculation_answer("4$5^2+7-------32   # +(-5)@1!")


def test_complicated_7():
    assert 53 == Calculation.calculation_answer("(((8-((5+33)-12)&6)^2)-7)+12@100")


def test_complicated_8():
    assert 168.35000000000002 == Calculation.calculation_answer("687*0.4+~102.45-(734.2#%12)")


def test_complicated_9():
    assert 33 == Calculation.calculation_answer("(6-3)!+45/6%5+0  ^23 - 18")


def test_complicated_10():
    assert -7721 == Calculation.calculation_answer("~-4+--45-5*-3-4-5-~-6^23#")


def test_complicated_11():
    assert 43 == Calculation.calculation_answer("-(5*3+2)$14+7&18#*2^3+(12)/3")


def test_complicated_12():
    assert -89 == Calculation.calculation_answer("~(8^2) + 12# - 5!&10*3 -- 2")


def test_complicated_13():
    assert -35 == Calculation.calculation_answer("103.6#*~10+104    $23 -6*3! -(89&4$3)+1")


def test_complicated_14():
    assert 41 == Calculation.calculation_answer("34444444356565.12###+100-~--(-4$30)*2")


def test_complicated_15():
    assert -10.18 == Calculation.calculation_answer("7.6+10.22%4-6^3&1-12 +-6/3")


def test_complicated_16():
    assert 10 == Calculation.calculation_answer("(12@3)$34-(132#+4!#+12)")


def test_complicated_17():
    assert -69 == Calculation.calculation_answer("~--3*4!-5%(3+11)/31#*11+14&(2^3/2)   +   12.75")


def test_complicated_18():
    assert 37208 == Calculation.calculation_answer("98334@-23946+~-542#-~3&4")


def test_complicated_19():
    assert 92 == Calculation.calculation_answer("-(-12&(12#!   +54%2/4-7*2))+100")


def test_complicated_20():
    assert -3283.66 == Calculation.calculation_answer("3333.2332#+23.1^2*-6-100$23--(7+53&3)%10@4")
